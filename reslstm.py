...
X_train, y_train, train_char= prepareData('maha_lng.txt')
...
model.add_node(TimeDistributed(Dense(train_char, activation='softmax', init= 'orthogonal')), inputs=['drop3','input'], merge_mode='sum', name='fc2')
...
model.save_weights('weights/weights_noep{0}_batch{1}_seq_{2}_loss{3}.h5'.format(no_epochs, batch, seq_length, score[0]), overwrite=True)
